...
| L40S (torch.compile) | 2.77x faster | SM 8.9 (Ada), torch.compile — MUCH faster than old L40S measurement |
| L40S (old, no compile?) | 1.06-1.15x faster | Old measurement, possibly without torch.compile or different driver |
...
Step times marked with * are **theoretical estimates**. RTX 5090 and L40S are now **measured**.
...
| L40S | 950 | 3.64 hr | **$1.02** | $0.037 | $0.0000739 |
...
- **L40S**: 950 ms/step — **2.5x faster than old measurement** (~2,400ms). torch.compile likely the difference.
...
**Key insight**: torch.compile delivers 2-4x speedup on this workload. The old L40S measurement (~2,400ms) was likely without torch.compile or with an older driver. The RTX 5090 at 605ms is only ~1.8x slower than 8xH100 (43ms × 8 GPUs = 344ms effective single-GPU).
...
- The old L40S measurement (~2,294-2,487ms) was likely without torch.compile or with older drivers. New L40S measurement (950ms) is 2.5x faster.
...
- RTX 3090 runs in eager mode (torch.compile disabled, SM 8.6). L40S (SM 8.9) and RTX 5090 (SM 12.0) use torch.compile.
- All remote GPUs are Novita cloud instances (proxy.us-ca-6.gpu-instance.novita.ai).
- Active GPU fleet (as of 2026-03-21): 1x local 3090, 1x remote 3090 (:62248), 1x L40S (:62396), 1x RTX 5090 (:62132).
