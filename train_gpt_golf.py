...
class Hyperparameters:
    ...
    # Depth recurrence: share weights across layers for more effective depth.
    num_unique_blocks = int(os.environ.get("NUM_UNIQUE_BLOCKS", 0))
    num_effective_layers = int(os.environ.get("NUM_EFFECTIVE_LAYERS", 0))

    # SwiGLU activation (replaces ReLU^2 MLP)
    use_swiglu = bool(int(os.environ.get("USE_SWIGLU", "0")))

    # Quantization-aware training
    qat_start_frac = float(os.environ.get("QAT_START_FRAC", -1.0))

    # LR schedule: "warmdown" (original) or "cosine"
    lr_schedule = os.environ.get("LR_SCHEDULE", "warmdown")
...
class CastedLinear(nn.Linear):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if self._qat_enabled and self.training and w.ndim == 2:
            # Fake int8 quantization with straight-through estimator (per-row)
            w32 = w.float()
            w_abs_max = w32.abs().amax(dim=1, keepdim=True).clamp(min=1e-8)
            scale = w_abs_max / 127.0
            w_q = (w32 / scale).round().clamp(-127, 127) * scale
            w = w + (w_q - w32).detach()  # STE: forward uses quantized, backward uses original
...
class SwiGLUMLP(nn.Module):
    # SwiGLU MLP: parameter-neutral replacement for ReLU^2
    # With mlp_mult=2: hidden = 4*dim/3 so gate+up+down = 3*(dim*4d/3) = 4d^2 params (same as ReLU^2)
    def __init__(self, dim: int, mlp_mult: int):
        super().__init__()
        hidden = (mlp_mult * dim * 2) // 3  # Keep param count ~equal
...
class GPT(nn.Module):
    def __init__(
        ...
        self.use_recurrence = num_unique_blocks > 0 and num_effective_layers > 0
        ...
        # Per-effective-layer scalar params (tiny — ~2K params each)
        self.attn_scales = nn.Parameter(torch.ones(self.total_layers, model_dim, dtype=torch.float32))
        self.mlp_scales = nn.Parameter(torch.ones(self.total_layers, model_dim, dtype=torch.float32))
        self.resid_mixes = nn.Parameter(
            torch.stack([torch.stack((torch.ones(model_dim), torch.zeros(model_dim))) for _ in range(self.total_layers)]).float()
        )
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            ...
            x = x + self.mlp_scales[i].to(dtype=x.dtype)[None, None, :] * block.mlp(block.mlp_norm(x))
            skips.append(x)
        ...
        for i in range(self.num_decoder_layers):
            ...
            mix = self.resid_mixes[eff_i].to(dtype=x.dtype)
            x = mix[0][None, None, :] * x + mix[1][None, None, :] * x0
            block = self._get_block(eff_i)
            attn_out = block.attn(block.attn_norm(x))
            x = x + self.attn_scales[eff_i].to(dtype=x.dtype)[None, None, :] * attn_out
            x = x + self.mlp_scales[eff_i].to(dtype=x.dtype)[None, None, :] * block.mlp(block.mlp_norm(x))
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        num_effective_layers=args.num_effective_layers,
    ...
    # Add GPT-level per-effective-layer scalars and skip weights
    ...
    def lr_mul(step: int, elapsed_ms: float) -> float:
        if args.lr_schedule == "cosine":
            # Cosine schedule with optional warmdown at end
            if max_wallclock_ms is not None:
                step_ms = elapsed_ms / max(step, 1)
                est_total = max_wallclock_ms / max(step_ms, 1e-9)
            else:
                est_total = float(args.iterations)
            frac = min(step / max(est_total, 1.0), 1.0)
            return 0.5 * (1.0 + math.cos(math.pi * frac))

        # Original warmdown schedule
    ...
    # QAT helper
    def set_qat(enable: bool) -> None:
        for module in base_model.modules():
            if isinstance(module, CastedLinear):
                module._qat_enabled = enable
    qat_enabled = False
    ...
    while True:
        ...
        if args.qat_start_frac >= 0 and not qat_enabled:
            if max_wallclock_ms is not None:
                train_frac = elapsed_ms / max_wallclock_ms
            else:
                train_frac = step / args.iterations
            if train_frac >= args.qat_start_frac:
                set_qat(True)
                qat_enabled = True
                log0(f"step:{step} enabling QAT (frac={train_frac:.3f})")
